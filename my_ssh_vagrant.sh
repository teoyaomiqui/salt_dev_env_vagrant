...
build_and_run_master(){
  ...
  if [[ "$(sudo docker images | grep -v $master_image_name)" ]]; then
    echo "Starting to build image ${master_image_name}"
    cmd="sudo docker build --tag ${master_image_name} ."
    echo $cmd
    $cmd 
  fi
  if [[ "$(sudo docker ps | grep -v ${master_container_name})" ]]; then
    ...
    sudo docker exec ${master_container_name} /usr/bin/python /usr/bin/salt-cloud --assume-yes --parallel -m /etc/salt/minion_map 
  fi
